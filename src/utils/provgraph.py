...
class RedProvGraph(ProvGraph):
    ...
    def add_from_graph(self, save_oprem, save_node_vid2score):
        for oprem in save_oprem:
            u = (oprem['u']).copy()
            v = (oprem['v']).copy()
            e = oprem['e']
            ...
            if oprem['u']['vid'] not in save_node_vid2score or oprem['v']['vid'] not in save_node_vid2score:
                continue
            u_score = save_node_vid2score[oprem['u']['vid']]
            v_score = save_node_vid2score[oprem['v']['vid']]

            for _u in u_node_list:
                for _v in v_node_list:
                    if 'score' not in self.nodes[_u] or self.nodes[_u]['score'] < u_score:
                        self.nodes[_u]['score'] = u_score
                    if 'score' not in self.nodes[_v] or self.nodes[_v]['score'] < v_score:
                        self.nodes[_v]['score'] = v_score
                    if any(d.get('type') == e['type'] for d in self.get_edge_data(_u, _v, default={}).values()):
                        continue
                    self.add_new_edge(_u, _v, e)
