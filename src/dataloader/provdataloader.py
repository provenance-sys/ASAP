...
class ProvDataset(Dataset):
        ...
        for _daily_data in self.splite_list:
            ...
            with open(_file, 'r') as f:
                for line in tqdm(f):
                    ...
                    if oprem['u']['type'] not in self.node_types or oprem['v']['type'] not in self.node_types or oprem['e']['type'] not in self.event_types:
                        continue
            ...
            for central_node_id in tqdm(central_node_ids):
                ...
                # get the negative subgraph sample of the central node
                if 'train' in self.splite_name:
    ...
    # @staticmethod
    # def filter_content(ntype, ncontent):
    #     content_parts = set(ncontent.split(';'))
    #     content_dir_parts = set()
    #     # if 'FILE' in ntype or 'RegistryKey' in ntype:
    #     #     for _part in content_parts:
    #     #         content_dir_parts.add(_part.rsplit("/", 1)[0] if "/" in _part else _part)
    ...
    def remove_duplicate_nodes_by_content(self, nx_graph: nx.MultiDiGraph, hops=3):
        ...
        # node_feat_save_lmdb = lmdb.open(self.node_feat_save_path, readonly=True, lock=False)
        # txn = node_feat_save_lmdb.begin()
        ...
        for node in nx_graph.nodes:
            if node == nx_graph.graph['central_node']:
                ...
                # nx_graph.nodes[node]['feat'] = pickle.loads(txn.get(str(nx_graph.nodes[node]['nid']).encode("utf-8")))
                continue
    ...
    def get_node_feature(self, ncontent: str, ntype: str) -> torch.Tensor:
        ...
        # return torch.tensor(node_features + node_encoding, dtype=torch.float32).reshape(-1)
