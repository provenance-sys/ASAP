...
class DarpaE3CadetsParser(BasicParser):
    ...
    def get_subject(self, entry_attrs):
        if entry_attrs['subject'] is None:
            return None
    ...
    def get_object(self, entry_attrs, predobject='predicateObject', predobjectpath='predicateObjectPath'):
        if entry_attrs[predobject] is None:
            return None
    ...
    def parse_netflowobject(self, entry: str) -> None:
        ...
        nodeid = entry_attrs['uuid']
        remoteaddr = entry_attrs['remoteAddress'].strip()
...
class DarpaE3TheiaParser(BasicParser):
    ...
    def get_object(self, entry_attrs, predobject='predicateObject'):
        if entry_attrs[predobject] is None:
            return None
        objectid = entry_attrs[predobject]['com.bbn.tc.schema.avro.cdm18.UUID']
        obj_dict = self.get(objectid)
